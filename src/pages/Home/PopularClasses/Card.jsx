...
const Card = ({item}) => {
  ...
  return (
    <div className='shadow-lg rounded-lg p-3 flex flex-col justify-between border border-secondary
    overflow-hidden m-4'>
      <img src={image} alt="" />
      <div className='p-4'>
        <h2 className='text-xl font-semibold mb-2 dark:text-white'>{name}</h2>
        <p className='text-gray-600 mb-2'>Total AVilable: {availableSeats}</p>
        <p className='text-gray-600 mb-2'>Price: {price}</p>
        
        <Link to={'/products'} className='text-center mt-2' >
